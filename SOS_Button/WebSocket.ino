  ...
  switch (type)
  {
    ...
    case WStype_TEXT:
    {
      ...
      if (!Po.success())
      {
        ...
        //Add truong Page vao Socket
        if (Po["Page"].success()) WsConnectPage(num, Po["Page"]); 
        DEBUGLOG_WEBSOCKET("\r\nJSON parsing Success!");
        //WsSendData((char*)"{\"CM\":\"S1\",\"Page\":1,\"Wifi\":{\"Ssid\":\"TienHuy\",\"Rssi\":90,\"Mac\":\"00:01:01:01:01:01\"},\"ST\":\"OK\",\"Erm\":\"JsonOK\"}",(PAGE_TAG_TypeDef)Po["Page"].as<uint8_t>());
        int n = WiFi.scanNetworks();
        if(n==0) return;
        for (int i = 0; i < n; ++i)
        {
          String WsRssi = "{\"CM\":\"S1\",\"Page\":1,\"Wifi\":{\"Ssid\":\"" + WiFi.SSID(i) 
          + "\",\"Rssi\":90,\"Mac\":\"" + WiFi.BSSIDstr(i) + "\"},\"ST\":\"OK\",\"Erm\":\"JsonOK\"}";
          WsSendData((char*)(WsRssi.c_str()),(PAGE_TAG_TypeDef)Po["Page"].as<uint8_t>());
          delay(10);
        }
      }      
    }
